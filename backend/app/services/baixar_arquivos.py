...
def baixar_arquivos_ultimos_trimestres(quantidade=3):
    ...
    if DEMOSTRACOES_PATH not in pastas:
        raise FileNotFoundError(f"A pasta {DEMOSTRACOES_PATH} não existe em {BASE_URL}")
    ...
    for ano in anos:
        ...
        zip_files = [f for f in arquivos_no_ano if f.endswith(".zip") and f[0].isdigit()]
        
        # Ordenar do mais recente para o mais antigo
        zip_files = sorted(zip_files, key=lambda x: (int(x[0]), int(x[-8:-4])), reverse=True)
    ...
    if len(arquivos_baixados) < quantidade:
        print(f"Atenção: Apenas {len(arquivos_baixados)} arquivos encontrados. Não foi possível completar {quantidade} trimestres.")

    return arquivos_baixados


if __name__ == "__main__":
    arquivos = baixar_arquivos_ultimos_trimestres()
